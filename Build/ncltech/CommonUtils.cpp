...
void CommonUtils::PositionCallback(GameObject* obj, Vector2 * posit){
	//*posit = Vector2(obj->Physics()->GetPosition().x, obj->Physics()->GetPosition().z);
	*posit = Vector2((*obj->Render()->GetChildIteratorStart())->GetWorldTransform().GetPositionVector().x, (*obj->Render()->GetChildIteratorStart())->GetWorldTransform().GetPositionVector().z);
}
	...
	const Vector4& color)
{
	...
	if (physics_enabled)
	{
		...
		pnode->SetCollisionRadius(radius);
	}
...
GameObject* CommonUtils::BuildPointObject(
	const std::string& name,
	const Vector3& pos,
	float radius,
	bool physics_enabled,
	...
	const Vector4& color)
{
	...
	// and a parent node that will contain the world transform/physics transform
	RenderNode* rnode = new RenderNode();
	OBJMesh * cb = new OBJMesh(MESHDIR"cube.obj");
	cb->SetTexture(CommonMeshes::CheckerboardTex());
	RenderNode* dummy = new RenderNode(cb, color);
	...
	if (physics_enabled)
	{
		...
		pnode->SetCollisionRadius(1.5f*sqrt(halfdims.x*halfdims.x+ halfdims.y*halfdims.y + halfdims.z*halfdims.z));
	}
...
GameObject* CommonUtils::BuildGround(
	const std::string& name,
	const Vector3& pos,
	const Vector3& halfdims,
	Vector2& posit,
	...
	const Vector4& color)
{
	...
	if (dragable)
	{
		ScreenPicker::Instance()->RegisterNodeForMouseCallback(
			dummy, //Dummy is the rendernode that actually contains the drawable mesh
			std::bind(&PositionCallback, obj, &posit)
		);
	}
