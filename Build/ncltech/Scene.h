...
public:
	...
	void set_Tree(bool ac) {
		PhysicsEngine::Instance()->tree->setActive(ac);
	}

	void updateTree() {
		//PhysicsEngine::Instance()->tree->draw();
		PhysicsEngine::Instance()->tree->empty(PhysicsEngine::Instance()->tree);
		for (std::vector<GameObject *>::iterator it = m_vpObjects.begin(); it != m_vpObjects.end(); ++it) {
			if ((*it)->HasPhysics()) {
				PhysicsEngine::Instance()->tree->push(*it);
			}
		}
	...
	void RemoveGameObject(GameObject* game_object)
	{
		if (game_object && game_object->scene == this)
		{
			...
			PhysicsEngine::Instance()->tree->pop(game_object);
		}
	}
